...
function check_prerequisites() {
    ...
    local CH_LIBUNRAR=$(check_libunrar)
    ...
    if [ "x${CH_LIBUNRAR}" == "x" ]
    then
        INSTALL_STR="${INSTALL_STR} libunrar5"
    fi
...
# pass mysql user password as second argument
# pass mysql host as third argument
function check_kronos_db () {
    local RES=0
    local MYSQL_USER=$1
    local MYSQL_PASS=$2
    local MYSQL_HOST=$3
    local DB_NAME='kronos_db'

    check_mysql_arguments ${MYSQL_USER} ${MYSQL_PASS} ${MYSQL_HOST}

    local CH_KRONOS_DB=$(mysql -u ${MYSQL_USER} -h ${MYSQL_HOST} -p${MYSQL_PASS} -Ns -e "show databases" | grep kronos)

    if [ "x${CH_KRONOS_DB}" == "x" ]
    then
        # Looks like database for kronos does not exist
        echo 'Looks like database for kronos does not exist, let creat it!'
        mysql -u ${MYSQL_USER} -h ${MYSQL_HOST} -p${MYSQL_PASS} -Ns -e "create database kronos_db CHARACTER SET utf8 COLLATE utf8_general_ci"
    else
        echo 'Database for cronos is exist, nothing to do.'
    fi
}
...
# pass mysql user password as second argument
# pass mysql host as third argument
# function will try to find gzipped dump in current directory
function restore_kronos_db() {
    local DUMP_NAME='kronos_db.sql.gz'
    local DB_NAME='kronos_db'
    local MYSQL_USER=$1
    local MYSQL_PASS=$2
    local MYSQL_HOST=$3
    ...
    if [ -e "./${DUMP_NAME}" ]
    then
        gunzip -k -c ./${DUMP_NAME} | mysql -u ${MYSQL_USER} -h ${MYSQL_HOST} -p${MYSQL_PASS} ${DB_NAME}
    else
        echo "Can't find kronos_db dump! Can't preocees, aborting."
        exit 1
    fi
...
function check_libunrar() {
    RES=0
    local CH_LIBUNRAR=$(dpkg -l | awk '{print $2}' | grep "^libunrar5")
